    ...
    <div class="kotak rounded-2 bg-silver3">
        ...
        <div class="row kotak">
            <div class="col-8 p-4 kotak shadow-keren">
                <div class="row d-flex align-items-start">
                    <div class="col-6">
                        <ul class="text-light fw-light">
                            <li>Berikut adalah data barang anda</li>
                        </ul>
                    </div>
                    <div class="col-6 text-end">
                        <a href="/rusaks" class="btn btn-sm bg-ijo3 mx-2 fw-semibold">Barang return</a>
                        <a href="/barangs/create" class="btn btn-sm bg-ijo fw-semibold">Tambah Barang</a>
                    </div>
                </div>
                @if ($barangs->count() > 0)
                <div class="bg-table">
                    <table class="table table-sm">
                        ...
                        <tbody>
                            ...
                            @php
                                $total = $barangs->currentPage() * $barangs->perPage();
                                $number = $total - $barangs->perPage() + 1 + $key;
                            @endphp
                                <td class="text-center">{{ $number  }}</td>
                                <td>{{ $barang->nama_barang }}</td>
                                <td class="text-center">{{ $barang->jumlah_barang }}</td>
                                <td class="text-center">{{ $barang->barang_masuk }}</td>
                                <td class="text-center">
                                    <form action="/rusaks/{{ $barang->id }}" method="POST">
                                        @csrf 
                                        @method('delete')
                                        <div class="">
                                        <a href="/rusaks/{{ $barang->id }}/edit" class="btn btn-sm bg-barak text-putih5 fw-semibold">Return barang</a>
                                        </div>
                                    </form>
                                </td>
                            </tr>
                ...
                <h1 class="text-white fs-4 px-5 pt-3">belum ada data barang !</h1>
                @endif
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination pagination-sm justify-content-end">
                        ...
                        @else
                        <li class="page-item">
                            <a class="page-link bg-ijo3 border border-white fw-semibold" href="{{ $barangs->previousPageUrl() }}">Previous</a>
                        </li>
                        @endif
                        ...
                        <li class="page-item">
                        <a class="page-link bg-ijo3 border border-white fw-semibold" href="{{ $barangs->nextPageUrl() }}">Next</a>
                        @else
                        <li class="page-item disabled">
